...
public class StringCal_sub {
	...
	public static void main(String[] args) {
		...
		String maxinum = str[0][0];
		String maxfnum = str[0][1];
		...
		if (str[1][0].equals("sub") && (args[0].contains(".") || args[2].contains("."))) {
			...
			for (int j = 0; j < le1F; j++) {
				m = Integer
						.toString(Character.getNumericValue(maxfnumc[l]) - Character.getNumericValue(minfnumc[l]) + s);
				if (m.length() > 1) {
		...
		if (str[1][0].equals("sub")) {
			...
			for (int j = 0; j < le1I; j++) {
				m = Integer
						.toString(Character.getNumericValue(maxinumc[l]) - Character.getNumericValue(mininumc[l]) + s);
				if (m.length() > 1) {
