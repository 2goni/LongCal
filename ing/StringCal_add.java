	...
	public void docal(StringCalVO vo) {
		...
		if (vo.getHasdot1() || vo.getHasdot2()) {
			for (int i = vo.getFnum1().length() - 1; i > 0; i--) {
				sum = Integer.toString(Character.getNumericValue(vo.getFnum1().charAt(i))
						+ Character.getNumericValue(vo.getFnum2().charAt(i)) + up);
				if (sum.length() > 1) {
		...
		for (int i = vo.getInum1().length() - 1; i >= 0; i--) {
			sum = Integer.toString(Character.getNumericValue(vo.getInum1().charAt(i))
					+ Character.getNumericValue(vo.getInum2().charAt(i)) + up);
			if (sum.length() > 1) {
				vo.setResult(sum.charAt(1));
				up = 1;
			} else {
