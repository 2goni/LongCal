...
public class StringCal {
	...
	public static void main(String[] args) {
		...
		//자릿수맞춤
		int s = 0;
		if(str[1][0].equals("add") && (args[0].contains(".") || args[2].contains("."))) {
			str[3][1] = "";
			int l = le1F-1;
			char[] floatnum1 = str[0][1].toCharArray();
			char[] floatnum2 = str[2][1].toCharArray();
			for(int j=0; j<le1F; j++) {
				m = Integer.toString(Character.getNumericValue(floatnum1[l]) + Character.getNumericValue(floatnum2[l])+s);
				if(m.length() > 1) {
					str[3][1] = m.charAt(1) + str[3][1];
					l--;
					s = 1;
				}else {
					str[3][1] = m + str[3][1];
